...
export async function POST() {
  ...
  const res = await fetch(
    "https://marriottarubasurfclub.ipoolside.com/api/palapa/get-palapas-special-days",
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        start_date: new Date().toISOString().slice(0, 10), //"2023-11-17",
        end_date: new Date().toISOString().slice(0, 10), //"2023-11-17",
        palapa_type_for_log: {
          id: 18, // 108 = 9, 208 = 18, 901 = 103
          typename: "Next day reservation", // "Multiple days reservation - 900 row ",
          count: 0,
          active: true,
          showmap: true,
          ruleNotSelectable: 0,
